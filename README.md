...
### ✅ 자동/수동 모드 기반 환기 제어
- 자동 모드: 미세먼지 농도에 따라 DC 모터 속도 자동 조절
- 수동 모드: 사용자가 속도 1~3단 직접 조절 가능

### ✅ 에너지 소비 측정 및 시각화
- 전류 센서를 통해 팬 작동 전력 측정, 절약량 시각화 제공

### ✅ 관리자/사용자 권한 분리
- 관리자는 여러 교실의 상태를 통합 관리
- 사용자는 자신이 속한 공간의 실내 환경을 확인

### ✅ 실시간 채팅 기능
- 사용자 ↔ 관리자 간 문의 및 피드백을 위한 채팅 시스템 내장

### ✅ 웹 기반 인터페이스 (React)
- 로그인, 대시보드, 데이터 분석, 제어, 설정 기능 탑재
